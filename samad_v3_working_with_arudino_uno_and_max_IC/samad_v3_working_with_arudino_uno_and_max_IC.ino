...
#define ROT_ENC_B 3
#define ROT_ENC_Z 4
Encoder encoder(ROT_ENC_A, ROT_ENC_B);
...
void setup() {
  ...
  // Initialize both displays with zeros
  lc.clearDisplay(0);

  while (analogRead(ROT_ENC_Z) < 0.0) {
    lc.clearDisplay(0);
    lc.setChar(0, 0, 'F', false); // Display "e" for error code 1
    lc.setChar(0, 1, 'A', false); // Display "r" for error code 1
...
void loop() {
    ...
    currentCount = encoder.read();

    // Check if the encoder value has changed
    if (currentCount != previousCount) {
        if ( display_1_state == SHOWING_COUNTER && display_2_state == SHOWING_COUNTER ) {
          err(4);
        }
